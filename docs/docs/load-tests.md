...
- For higher throughput, scale Philter horizontally and bring up additional proxy replicas. The proxy is stateless apart from its in-process concurrency slots, so replicas need no shared state.
