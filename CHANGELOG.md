...
- A `multipart/form-data` request body is now rejected with `400 invalid_request` / `unsupported_content_type` instead of `bad_json`. The body was never malformed JSON, so the old error sent operators looking for a problem that did not exist. The `/v1/files` message points at redacting file contents with Philter before upload; other multipart endpoints get a generic message. Audio (#40) and image edits (#41) are tracked for real support; file uploads are not (#42).
- Removed token-usage tracking entirely: the `prompt_tokens` and `completion_tokens` audit-log fields, the `philter_proxy_prompt_tokens_total` and `philter_proxy_completion_tokens_total` counters, and all per-provider usage parsing. Token counts are evidence of spend, not of redaction, and belong to the AI gateway alongside the quotas and rate limiting already removed. **Breaking** for anyone scraping those counters or reading those audit fields; use your gateway's accounting or the provider's usage dashboard. This also removes the `model` Prometheus label, the only label whose value came from the client, and with it the cardinality limiter that existed to stop a client exhausting a scraper's memory through it.
